...
from .application import UpdiApplication
...
class UpdiNvmProgrammer(object):
    ...
    def page_data(self, data, size):
        ...
        while len(result) < total_length / size:
            result.append(list(data[:size]))
            data = data[size:]
        return result

    def load_ihex_flash(self, filename):
        data, start_address = self._load_ihex(filename, self.device.flash_size, self.device.flash_start)
        return self.pad_data(list(data), self.device.flash_pagesize, 0xFF), start_address

    def load_ihex_eeprom(self, filename):
        data, start_address = self._load_ihex(filename, self.device.eeprom_size, self.device.eeprom_start)
        return self.pad_data(list(data), self.device.eeprom_pagesize, 0xFF), start_address
    ...
    def _load_ihex(self, filename, mem_size, mem_start):
        ...
        from ...ihex import IHexError, load_ihex

        try:
            img = load_ihex(filename)
        except IHexError as exc:
        ...
        self.logger.info(
            "Loaded {0:d} bytes from ihex starting at address 0x{1:04X}".format(len(data), start_address)
        )
